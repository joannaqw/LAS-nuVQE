...
molecule = MoleculeInfo(
    symbols=["H","H"],coords=([0.0, 0.0, 0.0], [0.5, 0.0, 0.0]),multiplicity=1,charge=0)
...
params=(np.random.uniform(0., 2.*np.pi, size=num_vqe_params)).tolist()
...
def cost_func2(params,hamiltonian, ansatz):
    expectation = qiskit_operator_energy(params, hamiltonian, ansatz)
    return expectation.values
#test = cost_func2(params,hamiltonian,ansatz)

# Running the VQE
t0 = time.time()
res = minimize(cost_func2,params,args = (hamiltonian,ansatz),method='L-BFGS-B',tol=0.00001,options={'disp':True})
t1 = time.time()
